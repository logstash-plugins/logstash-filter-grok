...
To ease migration to ECS the filter plugin is providing 2 sets of pattern files: the old 'legacy' set
and another set of patterns with captures (event field names) that are compliant with the schema.

The ECS set is a drop-in replacement - has all of the pattern definitions as the legacy files.
New features and enhancements will be primarily added to ECS compliant files, the legacy patterns might
still receive bug fixes which are backwards compatible.

To toggle between the two modes consult the <<plugins-{type}s-{plugin}-ecs_compatibility>> setting.
...
The value of this setting has an effect on extracted event field names when a composite pattern (e.g. `HTTPD_COMMONLOG`) is matched.
